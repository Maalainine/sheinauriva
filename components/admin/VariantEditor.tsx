...
function VariantCreator({
  ...
  const [rows, setRows] = React.useState<
    Array<{ typeId: string; optionId: string }>
  >([{ typeId: "", optionId: "" }]);
  ...
  const getAvailableTypes = (currentIdx: number) =>
    variantTypes.filter(
      (vt) => !rows.some((r, i) => i !== currentIdx && r.typeId === vt.id),
    );

  // Handler to change type or value in a row
  const handleRowChange = (
    idx: number,
    field: "typeId" | "optionId",
    value: string,
  ) => {
    setRows((rows) =>
      rows.map((r, i) =>
  ...
  const handleCreate = () => {
    ...
    const selectedOptions = rows.map((r) => ({
      variantTypeId: r.typeId,
      optionId: r.optionId,
    }));
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              <Select
                ...
                onValueChange={(value: string) =>
                  handleRowChange(idx, "typeId", value)
                }
              >
                ...
                <SelectContent>
                  {variantTypes
                    .filter(
                      (vt) =>
                        !rows.some((r, i) => r.typeId === vt.id && i !== idx),
                    )
                    .map((vt) => (
              ...
              <div className="flex-1 col-span-2">
                <Select
                  value={row.optionId}
                  onValueChange={(value: string) =>
                    handleRowChange(idx, "optionId", value)
                  }
                  disabled={!row.typeId}
                >
                  <SelectTrigger className="w-full">
                  ...
                  <SelectContent>
                    {row.typeId &&
                      (
                        variantTypes.find((vt) => vt.id === row.typeId)
                          ?.options ?? []
                      ).map((opt) => (
...
export const VariantEditor: React.FC<
  VariantEditorProps & { baseName?: string }
> = ({
  value,
  variantTypes,
  productPrice,
  ...
  const [selectedTypeIds, setSelectedTypeIds] = React.useState<string[]>(() => {
    ...
    const initial = Array.from(
      new Set(allowedOptions.map((o) => o.variantTypeId)),
    );
  ...
  const handleTypeToggle = (variantTypeId: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      const updatedOptions = allowedOptions.filter(
        (opt) => opt.variantTypeId !== variantTypeId,
      );
  ...
  allowedOptions.forEach((opt) => {
    if (!enabledOptionsByType[opt.variantTypeId])
      enabledOptionsByType[opt.variantTypeId] = [];
  ...
  const handleAdd = () => {
    ...
    const selectedOptions: VariantOption[] = Object.entries(
      enabledOptionsByType,
    ).map(([variantTypeId, optionIds]) => ({
      variantTypeId,
  ...
  const handleOptionChange = (
    ...
    const updated = value.map((v, i) => {
      ...
      const selectedOptions = v.selectedOptions
        .filter((opt) => enabledOptionsByType[opt.variantTypeId])
        .map((opt) =>
          opt.variantTypeId === variantTypeId ? { ...opt, optionId } : opt,
        );
  ...
  const handleAllowedOptionChange = (
    variantTypeId: string,
    optionId: string,
    checked: boolean,
  ) => {
    let updated = allowedOptions.filter(
      (opt) =>
        !(opt.variantTypeId === variantTypeId && opt.optionId === optionId),
    );
    if (checked) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                <span className="font-medium">{value.length}</span> variant
            ...
            <div className="rounded-lg border overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-border">
                  <thead className="bg-muted/5">
                    <tr>
                      {Object.entries(enabledOptionsByType).map(
                        ([variantTypeId]) => {
                                ...
                                <Select
                                  value={
                                    variant.selectedOptions.find(
                                      (opt) =>
                                        opt.variantTypeId === variantTypeId,
                                    )?.optionId || ""
                                  }
                                  onValueChange={(value) =>
                                    handleOptionChange(
              ...
              <div className="px-4 py-2 border-t bg-muted/5 text-xs text-muted-foreground">
                <div className="max-w-4xl mx-auto">
                  Each variant must have a unique combination of options. SKUs
                  are auto-generated from the product name and selected values.
                </div>
              </div>
            </div>
          )}

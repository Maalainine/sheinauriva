  ...
  callbacks: {
    async session({ session, token }: { session: DefaultSession, token: CustomJWT }) {
      if (session?.user) {
        ...
        const user = session.user as DefaultSession['user'] & { id?: string; role?: UserRole };
        user.id = token.id || token.sub || '';
        user.role = token.role;
      }
      return session;
    },
    async jwt({ token, user }: { token: JWT, user?: (NextAuthUser & { role?: UserRole }) | undefined }): Promise<CustomJWT> {
      const customToken = token as CustomJWT;
      if (user) {
        // Type assertion to handle the extended user type
        const extendedUser = user as NextAuthUser & { id: string; role?: UserRole };

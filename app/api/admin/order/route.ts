    ...
    try {
      ...
      const order = await prisma.$transaction(async (prisma) => {
        ...
        const shippingAddress =
          typeof data.shipping === "object" ? data.shipping.address : "";
        const shippingCity =
          typeof data.shipping === "object" ? data.shipping.city : "";
        ...
        const orderItemsData = data.items.map((item) => {
          ...
          if (!product) {
            throw new Error(
              `Product ${item.productId} not found in the database`,
            );
          }
          ...
          if (product.variants && product.variants.length > 0) {
            ...
            console.log(
              `Product ${item.productId} has variants, using variant:`,
              variantId,
            );
        ...
        if (session?.user?.id) {
          ...
          if (userExists) {
            ...
            console.warn(
              `User ${session.user.id} not found, proceeding as guest order`,
            );
          }
        }

        // Create the order and update user stats if applicable
        ...
        // Update user stats if this is an authenticated user order
        if (session?.user?.id && !isGuestOrder) {
          await prisma.user.update({
            where: { id: session.user.id },
            data: {
              totalSpent: {
                increment: subtotal + shippingCost,
              },
              ordersCount: {
                increment: 1,
              },
          ...
          console.log(
            `Updated user ${session.user.id} stats: +${subtotal + shippingCost} spent, +1 order`,
          );
        }

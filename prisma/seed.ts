...
async function main() {
  ...
  const clientUsers = [
    { email: 'alice@example.com', name: 'Alice Smith', password: await hash('password', 10) },
    { email: 'bob@example.com', name: 'Bob Johnson', password: await hash('password', 10) },
    { email: 'carol@example.com', name: 'Carol Williams', password: await hash('password', 10) },
  ];
  ...
  // 3. Create brands
  const brands = [
    { name: 'Nike', description: 'Just Do It', website: 'https://www.nike.com' },
    { name: 'Adidas', description: 'Impossible is Nothing', website: 'https://www.adidas.com' },
    { name: 'Puma', description: 'Forever Faster', website: 'https://www.puma.com' },
  ];
  ...
  for (const size of sizes) {
    await prisma.variantOption.upsert({
      where: { variantTypeId_value: { variantTypeId: sizeType.id, value: size } },
      update: {},
  ...
  for (const color of colors) {
    await prisma.variantOption.upsert({
      where: { variantTypeId_value: { variantTypeId: colorType.id, value: color } },
      update: {},
  ...
  const tshirt = await prisma.product.findFirst({ where: { name: 'Classic T-Shirt' } });
  const runningShoes = await prisma.product.findFirst({ where: { name: 'Running Shoes' } });
  ...
  if (tshirt) {
    ...
    if (sizeM && sizeL && colorRed && colorBlue) {
      ...
      const variants = [
        { sku: 'TSHIRT-RED-M', price: 29.99, stock: 50, selections: [colorRed.id, sizeM.id] },
        { sku: 'TSHIRT-RED-L', price: 29.99, stock: 40, selections: [colorRed.id, sizeL.id] },
        { sku: 'TSHIRT-BLUE-M', price: 29.99, stock: 45, selections: [colorBlue.id, sizeM.id] },
        { sku: 'TSHIRT-BLUE-L', price: 29.99, stock: 35, selections: [colorBlue.id, sizeL.id] },
      ];
